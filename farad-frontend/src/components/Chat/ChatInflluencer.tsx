...
const ChatInflluencer = ({ requestId }: any) => {
        ...
        <div className="flex flex-col p-4 h-[90vh]">
            <div className="flex-1 overflow-y-auto p-4 bg-[#00000050] rounded-md">
                {messages.map((chat:any, index:any) => (
                    <div
                        key={index}
                        className={`p-3 my-2 rounded-lg max-w-xs ${chat.user === 'influencer' ? 'ml-auto bg-blue-500 text-white' : 'mr-auto bg-gray-300'}`}
                    >
...
export default ChatInflluencer;
